...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    /* Create topic tree */
    Ouroborus::TopicTree topicTree([](Ouroborus::Subscriber *s, std::string_view message) {
    ...
    std::map<uint32_t, std::unique_ptr<Ouroborus::Subscriber>> subscribers;
        ...
        if (size > 4) {
            ...
            if (data[4] == 'S') {
                ...
                if (subscribers.find(id) != subscribers.end()) {
                    Ouroborus::Subscriber *subscriber = new Ouroborus::Subscriber(nullptr);
                    subscribers[id] = std::unique_ptr<Ouroborus::Subscriber>(subscriber);
