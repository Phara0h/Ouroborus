...
struct HttpResponseWrapper {
    ...
    template <bool SSL>
    static inline Ouroborus::HttpResponse<SSL> *getHttpResponse(const FunctionCallbackInfo<Value> &args) {
        Isolate *isolate = args.GetIsolate();
        auto *res = (Ouroborus::HttpResponse<SSL> *) args.Holder()->GetAlignedPointerFromInternalField(0);
        if (!res) {
            args.GetReturnValue().Set(isolate->ThrowException(String::NewFromUtf8(isolate, "Invalid access of discarded (invalid, deleted) Ouroborus.HttpResponse/SSLHttpResponse.", NewStringType::kNormal).ToLocalChecked()));
        }
        return res;
    }
    ...
    static void res_tryEnd(const FunctionCallbackInfo<Value> &args) {
        ...
        if (res) {
            ...
            /* This is a quick fix, it will need updating in Ouroborus later on */
    ...
    static Local<Object> init(Isolate *isolate) {
        ...
        if (SSL) {
            resTemplateLocal->SetClassName(String::NewFromUtf8(isolate, "Ouroborus.SSLHttpResponse", NewStringType::kNormal).ToLocalChecked());
        } else {
            resTemplateLocal->SetClassName(String::NewFromUtf8(isolate, "Ouroborus.HttpResponse", NewStringType::kNormal).ToLocalChecked());
        }
